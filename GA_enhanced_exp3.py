...
def reproduce(parent1, parent2, mutationRate):
    ...
    randomBit2 = randomBit
    while randomBit == randomBit2:
        randomBit2 = random.randint(1, len(parent1));
    ...
    child3.extend(parent1[:randomBit2]);
    child3.extend(parent2[randomBit2:]);
    child4.extend(parent2[:randomBit2]);
    child4.extend(parent1[randomBit2:]);
    ...
    for city in child4:
        if city not in seen:
            ...
            toSwap = child4[ind]
            child4[ind] = child3[dupes[dupesInd]];
            child3[dupes[dupesInd]] = toSwap;
            ind += 1;
            dupesInd += 1;

    length1 = Fitness(child1).routeDistance();
    length2 = Fitness(child2).routeDistance();
    length3 = Fitness(child3).routeDistance();
    length4 = Fitness(child4).routeDistance();

    minLength = min(length1, length2, length4, length3);
    if minLength == length1:
        newChild = child1;
    elif minLength == length2:
        newChild = child2;
    elif minLength == length3:
        newChild = child3;
    elif minLength == length4:
        newChild = child4;
    aRand = random.random();
