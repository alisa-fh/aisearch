...
def chooseParents(fitnessArr):
    ...
    parent = fitnessArr[len(fitnessArr) - 1];
    i = 0;
...
def fitness(population):
    ...
    for i in range(0, len(population)):
        ...
        sumFitness += fitnessResults[i];
    for i in range(0, len(fitnessResults)):
        deg = (fitnessResults[i] / sumFitness) * 360;
        if (i == 0):
            cumFitness[i] = deg;
        else:
            cumFitness[i] = cumFitness[i - 1] + deg;
    return cumFitness;
...
def reproduce(parent1, parent2, mutationRate):
    ...
    randomPartition1 = random.randint(1, len(parent1)); #pointer from 2nd item to last item
    randomPartition2 = randomPartition1;
    while randomPartition1 == randomPartition2:
        randomPartition2 = random.randint(1, len(parent1));
    for i in range(min(randomPartition1, randomPartition2), max(randomPartition1, randomPartition2)): #pointer from minimum of 2nd item to maximum of 2nd last
        child1[i] = parent2[i];
        child2[i] = parent1[i];
    ...
    for i in range(0, len(child1)):
        if child1[i] == -1:
            currentIndex = i;
            found = False;
            itemIndex = child1.index(parent1[currentIndex])
            while found == False:
                if child2[itemIndex] in child1:
                    itemIndex = child1.index(child2[itemIndex]);
                else:
                    found = True;
                    child1[currentIndex] = child2[itemIndex];

    for i in range(0, len(child2)):
        if child2[i] == -1:
            currentIndex = i;
            found = False;
            itemIndex = child2.index(parent2[currentIndex]);
            while found == False:
                if child1[itemIndex] in child2:
                    itemIndex = child2.index(child1[itemIndex]);
                else:
                    found = True;
                    child2[currentIndex] = child1[itemIndex];
...
def geneticAlgorithm(cityList, popSize, mutationRate, generations):
    ...
    for i in range(0, generations):
        if i % 2 == 0:
            fitnessArr = fitness(pop);
            newPop = [];
            for i in range(0, popSize):
                parent1 = pop[chooseParents(fitnessArr)];
                parent2 = pop[chooseParents(fitnessArr)];
