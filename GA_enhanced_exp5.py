...
def chooseParents(fitnessArr):
    ...
    i = 0;
    # print('deg is ', deg);
    # print('cumfitness is ', fitnessArr);
    while deg > fitnessArr[i]:
        i = i + 1;
    return i;


def distBetweenCities(fromCity, toCity):
        ...
        if self.distance == 0:
            ...
            for i in range(0, len(self.route)):
                ...
                if i + 1 < len(self.route):
                    ...
                    toCity = self.route[0];
                pathDistance += distBetweenCities(fromCity, toCity);
...
def fitness(population):
    ...
    for i in range(0, len(population)):
        arrayItem = [Fitness(population[i]).routeDistance(), i];
        fitnessResults.append(arrayItem);
        sumFitness += fitnessResults[i][0];
    for i in range(0, len(fitnessResults)):

        deg = (fitnessResults[i][0] / sumFitness) * 360;
        if cumFitness == []:
            cumFitness.append(deg);
        else:
            cumFitness.append(cumFitness[i - 1] + deg);
    fitnessResults.sort(key=lambda x: x[0]);
    # print('lenfitnessresults', len(fitnessResults));
    # print('lencumfitness', len(cumFitness));


    return cumFitness, fitnessResults;


def initialPopulation(cityList, popSize):
    population = []
    for i in range(0, popSize):
        population.append(random.sample(cityList, len(cityList)));  # appends a random route
    return population
...
def reproduce(parent1, parent2, mutationRate):
    ...
    newChild = childP1 + childP2

    # randomBit = random.randint(1, len(parent1));
    # child1.extend(parent1[:randomBit]);
    # child1.extend(parent2[randomBit:]);
    # child2.extend(parent2[:randomBit]);
    # child2.extend(parent1[randomBit:]);
    #
    # seen = {};
    # dupes = [];  # holds indices of values repeated in child1
    # ind = 0;
    # for city in child1:
    #     if city not in seen:
    #         seen[city] = 1;
    ...
    #     else:
    #         toSwap = child2[ind]
    #         child2[ind] = child1[dupes[dupesInd]];
    #         child1[dupes[dupesInd]] = toSwap;
    #         ind += 1;
    #         dupesInd += 1;
    #
    # length1 = Fitness(child1).routeDistance();
    # length2 = Fitness(child2).routeDistance();
    #
    # print('child1 premut', child1, 'child2 premut', child2);
    # if length1 > length2:
    #     newChild = child2;
    # else:
    #     newChild = child1;

    for swapped in range(len(newChild)):
        if (random.random() < mutationRate):
            swapWith = int(random.random() * len(newChild))

            city1 = newChild[swapped]
            city2 = newChild[swapWith]

            newChild[swapped] = city2
            newChild[swapWith] = city1

    #('newchild ', Fitness(newChild).routeDistance());
    return newChild;


def geneticAlgorithm(cityList, popSize, mutationRate, generations, eliteSize):
    pop = initialPopulation(cityList, popSize);
    bestLength = float("inf");
    for i in range(0, generations):
        cumFitness, fitnessArr = fitness(pop);
        lengthArr = [row[0] for row in fitnessArr]
        # fitnessAvArr = (np.array(fitnessArr)).mean(axis=0);
        # fitnessAv = fitnessAvArr[0];
        fitnessAv = np.quantile(lengthArr, 0.5);
        #print('fitnessAv ', fitnessAv);
        newPop = [];
        lenInNewPop = [];
        count = 0;
        for i in range(0, len(lengthArr)):
            if newPop.count(pop[fitnessArr[i][1]]) < 1 and lengthArr[i] <= fitnessAv:
                newPop.append(pop[fitnessArr[i][1]]);
                lenInNewPop.append(fitnessArr[i][0]);
                count += 1;
                if count >= eliteSize:
                    break;
        #print('post-elite ',lenInNewPop);
        for i in range(len(newPop), popSize):
            parent1 = pop[chooseParents(cumFitness)];
            parent2 = pop[chooseParents(cumFitness)];
            child = reproduce(parent1, parent2, mutationRate);
            childLength = Fitness(child).routeDistance();
            if childLength < bestLength:
                bestLength = childLength;
                bestTour = child;
            if childLength <= fitnessAv and newPop.count(child) < 1:
                newPop.append(child);
                lenInNewPop.append(childLength);
        #print('end ', lenInNewPop);
        pop = newPop.copy()
        #print('pop ', pop);
...
tour, tour_length = geneticAlgorithm(cityList, 100, 0.1, 500, 20);
