...
def reproduce(parent1, parent2, mutationRate):
    ...
    if aRand <= mutationRate:
        ...
        while (randInd2 == randInd1):
            randInd2 = random.randrange(len(newChild));
        mutatedChild = [];
        toScramble = newChild[min(randInd1, randInd2):max(randInd1, randInd2)];
        random.shuffle(toScramble);
        mutatedChild.extend(newChild[:min(randInd1, randInd2)]);
        mutatedChild.extend(toScramble);
        mutatedChild.extend(newChild[max(randInd1, randInd2):]);
        return mutatedChild;
    return newChild;
