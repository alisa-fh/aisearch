    ...
    for i in range(0, generations):
        if i % 2 == 0:
            ...
            for i in range(eliteSize, popSize):
                ...
                newPop.append(child);
        if i % 2 == 1:
            cumFitness, fitnessArr = fitness(newPop);
            pop = [];
            for i in range(0, eliteSize):
                pop.append(newPop[fitnessArr[i][1]]);
            for i in range(eliteSize, popSize):
                parent1 = newPop[chooseParents(cumFitness)];
                parent2 = newPop[chooseParents(cumFitness)];
                child = reproduce(parent1, parent2, mutationRate);
                childLength = Fitness(child).routeDistance();
                if childLength < bestLength:
                    bestLength = childLength;
...
tour, tour_length = geneticAlgorithm(cityList, 100, 0.01, 500, 20);
